...
/////////////////
const BotName = 'RMS BOT'; 
const instagram = 'https://instagram.com/itsmeikyxsec404'; 
const whatsapp = 'wa.me/6281281872699'; 
...
conn.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log('Bot by ig:@itsmeikyxsec404')
conn.on('message-status-update', json =>
{
   const participant = json.participant ? ' (' + json.participant + ')' : '' // participant exists when the message is from a group
   console.log(`[ ${moment().format("HH:mm:ss")} ] => bot by ig:@itsmeikyxsec404`)
})
...
// Mempertahankan sesi agar tetap nyala
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Hai minna~, Im RMS BOT. To find out the commands on this bot type ${prefix}menu`)
        })
	    }
	}
    })
